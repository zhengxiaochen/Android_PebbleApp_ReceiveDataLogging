...
public class MainActivity extends Activity { 
    ...
    private PebbleKit.PebbleDataLogReceiver mDataLogReceiver = new PebbleKit.PebbleDataLogReceiver(PEBBLE_DATA_APP_UUID) {    
    	Lock lock= new ReentrantLock(true); //Define a lock to avoid the concurrency problem when writing data to txt file
       	...
       	public void receiveData(Context context, UUID logUuid, UnsignedInteger timestamp, UnsignedInteger tag, byte[] accdata) {         	 	   	
			...
			//dataString=dataString.substring(0, dataString.indexOf("}")+1); //Delete the spaces and duplicated }s at the end of string.
			//dataString=dataString.substring(0, dataString.indexOf("}"))+ getLocation()+phoneAcc+",'UUID':"+uuid+",'asasec_op':ALTA"+",'asasec_ev':ID01"+"}";	
   ...
   */
    ...
    //private String folderPath =File.separator + "tmp" +  File.separator + "cache"+ File.separator;
    ...
    protected void onResume() {    	
      ...
      //****************creat log file************
        ...
        if (mDataLogReceiver != null && !registered) {
        ...
        }else if (mDataLogReceiver == null){
        	textView.append("Can not receive data because mDataLogReceiver is NULL !!\n");
        	//**************writing to log*****
        	BufferedWriter logWritter = new BufferedWriter(fLog);
			try {
				logWritter.write("Can not receive data because mDataLogReceiver is NULL !!\n");
				logWritter.close();
			} catch (IOException e) {
				textView.setText("Error writing to and closing file:"+e.getMessage());				
				return;
        	...
        	textView.append("Can not receive data because parameter registered is wrong!!\n");
        }
